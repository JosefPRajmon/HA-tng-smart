...
from .const import CONF_HEAT_PUMP_HASH, DOMAIN, MAX_HEAT_TEMP, MAX_THERMOSTAT_TEMP, MIN_HEAT_TEMP, MIN_THERMOSTAT_TEMP
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities):
    ...
    async_add_entities([
        ...
        TngHeatWaterTemp(coordinator, entry),
    ])
...
class TngHeatWaterTemp(CoordinatorEntity[TngCoordinator], NumberEntity):
    """Teplota vody v trubkách (výstup čerpadla) - technický parametr, ne
    pokojová teplota. Vyšší hodnota neznamená 'víc topí', jen zvyšuje
    teplotu média (a snižuje účinnost/COP čerpadla) - typicky se nastaví
    jednou při zprovoznění podle typu topení (podlahovka vs. radiátory) a
    dál se nemění. Proto je to schované jako pokročilé nastavení, ne v
    hlavní climate kartě."""

    _attr_has_entity_name = True
    _attr_name = "Teplota vody v trubkách (pokročilé)"
    _attr_icon = "mdi:pipe"
    ...
    def __init__(self, coordinator: TngCoordinator, entry: ConfigEntry):
        ...
        self._entry = entry
        self._pending = False
        self._attr_unique_id = f"{entry.data[CONF_HEAT_PUMP_HASH]}_heat_water_temp"
